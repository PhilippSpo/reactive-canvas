...
ReactiveCanvas.prototype.createElementAtOrigin = function() {
	...
	if(this.clippedShape){
		origin.x = this.clippedShape.coords[0].x;
		origin.y = this.clippedShape.coords[0].y;
		topLeft = origin;
		topRight = {
			x: origin.x + length,
			y: origin.y
		...
		topLeft = CanvasFunctions.transformCoord(this.canvas, {
			x: origin.x,
			y: origin.y
		}, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
		topRight = CanvasFunctions.transformCoord(this.canvas, {
			x: origin.x + length,
			y: origin.y
		}, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
		bottomRight = CanvasFunctions.transformCoord(this.canvas, {
			x: origin.x + length,
			y: origin.y + length
		}, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
		bottomLeft = CanvasFunctions.transformCoord(this.canvas, {
			x: origin.x,
			y: origin.y + length
		}, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
	}
	...
	CanvasFunctions.insertPolygon(polyCoords, this.polyCollection, this.extendPolygon);
};