...
ReactiveCanvas.prototype.selectShape = function(shape) {
	...
	if (currentSelected && (currentSelected.dragging === true)) {
		return;
	}
	self.resetSelection();
...
ReactiveCanvas.prototype.setupReactiveUpdates = function() {
	...
	_.each(self.shapeConfig, function(cfg) {
		...
		var shapeClass = eval(cfg.type);
		// observe added shapes
		shapes.observeChanges({
			added: function(id) {
				self.addShape(new shapeClass(id, cfg, self));
			},
			removed: function(id){
				self.removeShape(id);
			}
...
ReactiveCanvas.prototype.createShape = function(shapeType, mouseEvent, extendShape) {
	...
	var shapeFunctions = ReactiveCanvasStore.shapeFunctions[shapeType];
	...
	shapeFunctions.createStandardShape(cfg, coord, transformCoord, self.panZoomLayer.scaleFactor, extendShape);
