...
ReactiveCanvas.prototype.dblclick = function(e) {
	...
	var mouse = CanvasFunctions.getMouse(e, this.translation, this.scaleFactor);
...
ReactiveCanvas.prototype.initEventHandlers = function() {
	...
	function mousedown(e) {
		e.preventDefault();
		e.stopPropagation();
		var mouse = CanvasFunctions.getMouse(e, myState.translation, myState.scaleFactor);
	...
	if (!this.valid || redrawAsChild) {
		...
		for (var i = 0; i < l; i++) {
			...
			if (mySel !== shape) {
				...
				if (shape.x > (this.canvas.width-this.translation.x) || shape.y > (this.canvas.height-this.translation.y) ||
