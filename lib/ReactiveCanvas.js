...
ReactiveCanvas = function(canvas, rectCollection, polyCollection) {
	...
	rectangles.observeChanges({
		added: function(id) {
			myState.addShape(new Rectangle(id, myState.rectCollection, myState));
		},
		// removed: function(id) {
	...
	polygons.observeChanges({
		added: function(id) {
			myState.addShape(new Polygon(id, myState.polyCollection, myState));
		},
		// removed: function(id) {
			// is handled automatically at the moment
			// if you want to have some code to handle the removing do it here
	...
	// Right here "this" means the ReactiveCanvas. But we are making events on the Canvas itself,
	// and when the events are fired on the canvas the variable "this" is going to mean the canvas!
	// Since we still want to use this particular ReactiveCanvas in the events we have to save a reference to it.
	...
	function dblclick(e) {
		...
		if (myState.insertMode === 'rect') {
			...
			counter = myState.rectCollection.find().fetch().length;
			myState.rectCollection.insert({
				coords: {
					x: mouse.x - 10,
					y: mouse.y - 10,
					w: 100,
...
ReactiveCanvas.prototype.drawRectWithBorder = function(x, y, sideLength, ctx, borderColor, bgColor) {
	if (!borderColor) {
		borderColor = 'black';
	}
	if (!bgColor) {
