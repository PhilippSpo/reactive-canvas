...
ReactiveCanvas = function(canvas, rectCollection, polyCollection, imageSrc) {
	// **** First some setup! ****
	...
	this.panZoomLayer = new PanZoomCanvas(canvas, imageSrc, this);
};
...
var mousemove = function(e) {
	...
	var mouse = CanvasFunctions.getMouseWithContextTranslation(e, self.panZoomLayer.translation, self.panZoomLayer.scaleFactor, self.canvas);
...
var mousedown = function(e) {
	...
	self.mouseDown = true;
	var mouse = CanvasFunctions.getMouseWithContextTranslation(e, self.panZoomLayer.translation, self.panZoomLayer.scaleFactor, self.canvas);
	self.mouseDownTimer = (new Date().getTime()) / 10;
	self.mouseDownPos = mouse;
...
var mouseup = function(e) {
	...
	var mouse = CanvasFunctions.getMouseWithContextTranslation(e, self.panZoomLayer.translation, self.panZoomLayer.scaleFactor, self.canvas);
	// setTimeout(function() {
		// console.log('get self.activeDblClick', self.activeDblClick);
		// if(self.activeDblClick === true){
		// 	self.activeDblClick = false;
		// }
		...
		// check time on mouse down timer to be less than 1/10 second
		var currentTime = (new Date().getTime()) / 10;
		if ((currentTime - self.mouseDownTimer) > 40) {
			return;
		}
		if (mouse.x - self.mouseDownPos.x > 2 || mouse.y - self.mouseDownPos.y) {
			return;
		}
...
ReactiveCanvas.prototype.setShapeForCropping = function(shapeId) {
	...
	var shape = this.getShapeForId(shapeId);
	this.clippedShape = shape;
	// set shape invisible
	shape.visible = false;
	// calc the bounding rectangle for the canvas
	this.boundingRect = CanvasFunctions.calcClipRectangle(shape);
	// change canvas dimensions to fit the shape
	canvas.width = this.boundingRect.w + 1;
	canvas.height = this.boundingRect.h + 1;
	// translate context to shape
	this.panZoomLayer.translation = {
		x: -this.boundingRect.x,
		y: -this.boundingRect.y
	};
	this.panZoomLayer.translate(this.panZoomLayer.translation);
...
ReactiveCanvas.prototype.clear = function() {
	...
	var canvasOrigin = {
		x: -this.panZoomLayer.translation.x,
		y: -this.panZoomLayer.translation.y
	};
	var canvasExpansion = {
		x: -this.panZoomLayer.translation.x + canvas.width,
		y: -this.panZoomLayer.translation.y + canvas.height
	};
	var transformedOrigin = CanvasFunctions.transformCoord(this.canvas, canvasOrigin, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
	var transformedExpansion = CanvasFunctions.transformCoord(this.canvas, canvasExpansion, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
...
ReactiveCanvas.prototype.cleanup = function() {
	...
	this.panZoomLayer.resetTransformation();
	_.each(this.shapes, function(shape) {
		shape.visible = true;
	});
	this.boundingRect = {
		x: 0,
		y: 0,
		w: this.panZoomLayer.imageObj.width,
		h: this.panZoomLayer.imageObj.height
	};
};
...
/*
 * creates a polygon at the origin of the viewport
 */
ReactiveCanvas.prototype.createElementAtOrigin = function() {
	var origin = this.getOrigin();
	var length = 200 * this.panZoomLayer.scaleFactor;
	var rectangluarCoords = [];

	// wrap transform coord function to only make the coord variabl
	function transformCoord (coord){
		return CanvasFunctions.transformCoord(this.canvas, coord, this.panZoomLayer.scaleFactor, this.panZoomLayer.translation);
	}
	// check if the context is a clipped shape
	if(this.clippedShape){
		rectangluarCoords = CanvasFunctions.calculateSquare(this.clippedShape.coords[0], length, transformCoord);
	}else{
		rectangluarCoords = CanvasFunctions.calculateSquare(origin, length, transformCoord);
	}
	// insert polygon
	CanvasFunctions.insertPolygon(rectangluarCoords, this.polyCollection, this.extendPolygon);
};