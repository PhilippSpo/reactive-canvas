...
PanZoomCanvas.prototype.clear = function(canvas, ctx) {
	...
	ctx.strokeRect(transformedOrigin.x, transformedOrigin.y, transformedDimension.w, transformedDimension.h);
...
PanZoomCanvas.prototype.setScale = function(scale) {
	...
	if (canvas.width > boundingRect.w*scale || canvas.height > boundingRect.h*scale) {
		return;
	}
...
PanZoomCanvas.prototype.getBoundingRectWithOverhead = function() {
	...
	var overhead = {
		x: this.reactCanvObj.boundingRect.w * 0.1,
		y: this.reactCanvObj.boundingRect.h * 0.1
	};
	var boundingRect = {
