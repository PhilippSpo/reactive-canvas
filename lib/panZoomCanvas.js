...
PanZoomCanvas.prototype.clear = function(canvas, ctx) {
	...
	var canvasExpansion = {
		x: canvas.width/this.scaleFactor,
		y: canvas.height/this.scaleFactor
	};
...
PanZoomCanvas.prototype.setScale = function(scale) {
	...
	if (canvas.width-100 > currentImageWidth || canvas.height-100 > currentImageHeight) {
		return;
	}
...
PanZoomCanvas.prototype.checkScale = function(scale) {
	...
	var boundingRect = {
		x: this.reactCanvObj.boundingRect.x - 100,
		y: this.reactCanvObj.boundingRect.y - 100,
		w: this.reactCanvObj.boundingRect.w + 200,
		h: this.reactCanvObj.boundingRect.h + 200
	};

	if (CanvasDimensions.x > boundingRect.w || CanvasDimensions.y > boundingRect.h) {
		return false;
	}
	return true;
};
