...
Polygon.prototype.drawHandles = function(ctx) {
  for (var i in this.coords) {
    if (this.coords.hasOwnProperty(i)) {
      ...
      this.canvasStateObj.drawRectWithBorder(point.x, point.y, this.closeEnough, ctx, borderColor, bgColor);
    }
  }
};
...
Polygon.prototype.touchedAtHandlesReturnPoint = function(mx, my) {
  ...
  for (var i in this.coords) {
    if (this.coords.hasOwnProperty(i)) {
      var point = this.coords[i];
      if (this.canvasStateObj.checkCloseEnough(mx, point.x, this.closeEnough) && this.canvasStateObj.checkCloseEnough(my, point.y, this.closeEnough)) {
        return point;
      }
    }
  }
...
var addPointToPolyDoc = function(newPoint, canvasStateObj, polygonId, polyCollection) {
