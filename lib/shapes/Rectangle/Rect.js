...
Rectangle = function(id, cfg, canvasStateObj) {
  var shape = cfg.collection.findOne({
    _id: id
  });
  if (!shape) {
    return;
  ...
  this.textField = new TextField(this.name, this.canvasStateObj, cfg.collection, this.id);
  ...
  Tracker.autorun(function() {
    var shape = self.collection.findOne({
      _id: self.id
    });
    if (!shape) {
      self.remove();
...
Rectangle.prototype.draw = function(ctx) {
  ...
  if (this.selected === true && this.cfg.resizeable !== false) {
...
Rectangle.prototype._mouseDown = function(e, mouse) {
  ...
  if (this.touchedAtHandles(mouse.x, mouse.y) && this.cfg.resizeable !== false) {
    // resizing
    ...
    if (this.contains(mouse.x, mouse.y)) {
      ...
      if (this.textField.selected && this.textField.contains(mouse.x, mouse.y)) {
        this.textField._mouseDown(e, mouse);
      } else {
...
Rectangle.prototype._mouseUp = function(e, mouse) {
  ...
  if (!this.selected) {
    ...
    touchHandled = this.textField._mouseUp(e, mouse);
  }
  if (this.contains(mouse.x, mouse.y)) {
    touchHandled = true;
  }
...
Rectangle.prototype.mouseDownResize = function(e, mouse) {
  ...
  this.canvasStateObj.needsRedraw(); // something is resizing so we need to redraw
};
...
Rectangle.prototype._deselect = function() {

};
Rectangle.prototype._select = function() {
