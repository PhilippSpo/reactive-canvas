...
CanvasFunctions = {
	...
	getMouse: function(coord, translation, scale, canvasId) {
		var canvas = document.getElementById(canvasId);
	...
	// 	shape.draw(ctx);
	// },
	clipShapeOnCanvas: function(ctx, shape) {
		...
		if (shape instanceof Polygon) {
			...
			ctx.moveTo(shape.x + 10, shape.y);
			ctx.lineTo(shape.x + 10, shape.y+shape.h);
			ctx.lineTo(shape.x - 10+shape.w, shape.y+shape.h);
			ctx.lineTo(shape.x - 10+shape.w, shape.y);
			// ctx.rect(shape.x + 20, shape.y + 20, shape.w - 40, shape.h - 40);
			ctx.stroke();
			ctx.restore();
	...
	calculateSquare: function(origin, length, transform) {
		...
		function addCoord(i, j) {
			var coord = {
				x: parseFloat(origin.x + (i * length)),
				y: parseFloat(origin.y + (j * length))
			};
